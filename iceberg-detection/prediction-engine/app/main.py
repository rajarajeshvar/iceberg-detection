...
# Initialize SegFormer-B0 SAR Segmentation Pipeline
seg_ckpt_path = os.getenv("SEGFORMER_CHECKPOINT", "./models/segformer_best_model.pt")
try:
    seg_pipeline = SARIcebergSegmentationPipeline(checkpoint_path=seg_ckpt_path)
    print("[Main] SegFormer-B0 SAR Segmentation Pipeline loaded successfully.")
except Exception as e:
    print(f"[Main] Warning: Failed to initialize SegFormer pipeline: {e}")
    seg_pipeline = None
...
container["segmentation_pipeline"] = seg_pipeline
...
app = FastAPI(
    ...
    description=(
        ...
        "Provides satellite Sentinel-1 SAR iceberg segmentation (SegFormer-B0), 6h-72h trajectory predictions (Feature 2), "
